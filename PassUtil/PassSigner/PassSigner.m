...
+ (SecIdentityRef)passSigningIdentityRef:(NSString*)passTypeIdentifier
{
    ...
    NSDictionary* matchingDictionary = @{ (NSString*)kSecClass : (NSString*)kSecClassIdentity, (NSString*)kSecMatchSubjectContains : passTypeIdentifier, (NSString*)kSecReturnRef: (NSNumber*)kCFBooleanTrue};
    CFTypeRef result;
...
+ (BOOL)signPassWithURL:(NSURL *)passURL certSuffix:(NSString*)certSuffix outputURL:(NSURL *)outputURL zip:(BOOL)zip {
    ...
    if (zip) {
        ...
        [zipTask setStandardOutput:[NSFileHandle fileHandleWithStandardOutput]];
        [zipTask setStandardError:[NSFileHandle fileHandleWithStandardError]];
        // Fire and wait.
        [zipTask launch];
